...
public class DryingRackSerializer implements RecipeSerializer<DryingRackRecipe> {
	...
	public DryingRackRecipe fromJson(ResourceLocation pRecipeId, JsonObject pJson) {
		...
		if (pJson.get("result").isJsonObject())
			...
			itemstack = new ItemStack(Optional.ofNullable(ForgeRegistries.ITEMS.getValue(resourcelocation))
	...
	@Override
	public DryingRackRecipe fromNetwork(ResourceLocation resourceLocation, FriendlyByteBuf friendlyByteBuf) {
		String s = friendlyByteBuf.readUtf();
		Ingredient ingredient = Ingredient.fromNetwork(friendlyByteBuf);
		ItemStack itemstack = friendlyByteBuf.readItem();
		float f = friendlyByteBuf.readFloat();
		int i = friendlyByteBuf.readVarInt();
		return new DryingRackRecipe(resourceLocation, s, ingredient, itemstack, f, i);
	}
